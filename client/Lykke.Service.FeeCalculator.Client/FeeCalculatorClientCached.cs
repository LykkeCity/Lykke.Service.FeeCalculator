...
namespace Lykke.Service.FeeCalculator.Client
{
    public sealed class FeeCalculatorClientCached : IFeeCalculatorClient
    {
        ...
        {
            var key = KeyGenerator.GetKeyForBankCard();
            if (_cache.TryGetValue<BankCardsFeeModel>(key, out var bankCardsFee))
            {
                return bankCardsFee;
            }

            var newBankCardsFee = await _client.GetBankCardFees();
            _cache.Set(key, newBankCardsFee, _expirationPeriod);
            return newBankCardsFee;
        }
