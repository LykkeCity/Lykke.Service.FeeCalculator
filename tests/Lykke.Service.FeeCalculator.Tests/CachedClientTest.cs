...
namespace Lykke.Service.FeeCalculator.Tests
{
    public class CachedClientTest : IDisposable
    {
        ...
        [Fact]
        public async Task ShouldCacheGetBankCardFees()
        {
            await SetupBankCard();

            await _client.ReceivedWithAnyArgs(1).GetBankCardFees();
        }

        [Fact]
        public async Task ShouldRefreshCacheGetBankCardFees()
        {
            await SetupBankCard();
            ...
            await _client.ReceivedWithAnyArgs(2).GetBankCardFees();
        }
        ...
        {
            IReadOnlyList<CashoutFee> fee = new List<CashoutFee> { new CashoutFee() };
